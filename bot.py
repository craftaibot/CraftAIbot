...
async def start(event):
    ...
    if is_member:
        buttons = [
            ...
            [Button.url("Twitter", "https://twitter.com/your_twitter_handle")],  # Replace with your Twitter link
            [Button.url("Telegram Group", "https://t.me/your_telegram_group")],  # Replace with your Telegram group link
    ...
    msg = gif_url = "http://www.master-list2000.com/abillmiller/gifs/ascii_p11.gif"
    await event.client.send_file(event.chat_id, gif_url, caption="Welcome. Use the Edit AI Agent button to manage the existing Telegram user accounts linked to this bot or Create AI Agent button to link a Telegram user account.",
    buttons=buttons
)
...
async def process_personality_samples(event, user_id, telegram_id, chat_group_id, samples_text):
    """
    Generates a personality description by calling the OpenAI API.
    """
    try:
        ...
        payload = {
            ...
            "prompt": f"Analyze the following text and describe the personality of the speaker:\n{samples_text}\n\nPersonality Description:",
            "max_tokens": 200,
            "temperature": 0.7,
        }
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(openai_api_url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    personality_description = response_data.get("choices", [{}])[0].get("text", "").strip()
                else:
                    logging.error(f"OpenAI API call failed with status {response.status}: {await response.text()}")
        ...
        msg = await event.respond(f"Generated personality:\n\n{personality_description}", buttons=buttons)
        last_bot_message_id[user_id] = msg.id
